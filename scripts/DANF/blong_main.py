...
def dnn_train_main(extf, name="dnn"):
    dataset_prefix = "white_noise_{}e_".format(extf)
    train_dataset = ble_data_loader.ble_raw_dataset(dataset_prefix + "train", extf, [-20, 5])
...
def dnn_wr_update_main(extf, name="dnn_uni"):
    syms = []
    labels = []
    dis = []
    test_syms = []
...
if __name__ == "__main__":
    db_ranges = []
    for extf in [2, 4, 16, 32, 64]:
        print("+++++++++++++++++++++++++++++++")
        print(extf)
        dnn_train_main(extf, "dnn_uni")
        print("+++++++++++++++++++++++++++++++")
